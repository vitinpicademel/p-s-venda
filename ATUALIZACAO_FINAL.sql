-- ============================================
-- VERSÃO FINAL DEFINITIVA - Execute Passo a Passo
-- ============================================

-- PASSO 1: Adicionar apenas as colunas necessárias
...
-- PASSO 2: Criar tabela simples para documentos
...
-- PASSO 5: Política básica para admins
...
-- PASSO 6: Criar função simples para as 9 etapas
CREATE OR REPLACE FUNCTION create_default_process_steps()
RETURNS TRIGGER AS $$
BEGIN
  -- Limpar etapas antigas se existir
...
CREATE TRIGGER on_process_created
  ...
  FOR EACH ROW EXECUTE FUNCTION create_default_process_steps();

-- PASSO 8: Migrar processos existentes (se houver erro aqui, pode ignorar este passo)
-- Este passo é opcional - se der erro, continue sem ele
DO $$
BEGIN
  INSERT INTO process_steps (process_id, step_order, step_name, step_description, status)
  SELECT 
    ...
    CASE step_order
      ...
      WHEN 5 THEN 'Solicitação de vistoria enviada ao banco'
...
EXCEPTION
  WHEN OTHERS THEN
    NULL; -- Ignorar erros na migração
END $$;
